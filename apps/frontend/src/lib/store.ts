...
export async function resetStoreSync() { cachedProjects = []; cachedNotes = []; cachedTimeline = []; }

async function load() {
  const user = await getSession();
  if (!user) throw new Error("Sign in with GitHub to access DevLog");
  const response = await api(`/api/state?userId=${encodeURIComponent(user.id)}`);
  ...
  cachedNotes = result.data.notes.map((note: any) => ({ id: note.id, title: note.title || "Untitled Note", content: note.content, createdAt: note.createdAt, updatedAt: note.updatedAt }));
  cachedTimeline = result.data.timeline.map((event: any) => ({ id: event.id, type: event.eventType, title: event.payload?.title || event.eventType, description: event.payload?.description || "", image: event.payload?.image, projectId: event.projectId || undefined, projectName: event.payload?.projectName, timestamp: event.occurredAt }));
...
async function save() {
  ...
  setStatus("syncing");
  try {
    const response = await api("/api/state", { method: "PUT", body: JSON.stringify({ userId: user.id, projects: cachedProjects, notes: cachedNotes, timeline: cachedTimeline }) });
    const result = await response.json();
...
export const store = {
  async getProjects() { if (!cachedProjects.length) await load(); return getCachedProjects(); },
  async saveProjects(projects: Project[]) { cachedProjects = structuredClone(projects); notify("projects"); await save(); },
  async getNotes() { if (!cachedNotes.length) await load(); return getCachedNotes(); },
  async saveNotes(notes: Note[]) { cachedNotes = structuredClone(notes); notify("notes"); await save(); },
  async getTimeline() { if (!cachedTimeline.length) await load(); return getCachedTimeline(); },
  async saveTimeline(timeline: TimelineEvent[]) { cachedTimeline = structuredClone(timeline); notify("timeline"); await save(); },
  async addTimelineEvent(event: Omit<TimelineEvent, "id" | "timestamp">) { const value = { ...event, id: crypto.randomUUID(), timestamp: new Date().toISOString() } as TimelineEvent; cachedTimeline = [value, ...cachedTimeline]; notify("timeline"); void save(); return value; },
